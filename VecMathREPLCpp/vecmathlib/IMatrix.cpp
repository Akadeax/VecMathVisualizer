...
IMatrix::IMatrix(Type type, AddressMode mode, int rows, int columns) : IMatrix(type, mode, 0, rows, columns) {}
...
IMatrix* IMatrix::inverse()
{
    ...
    IMatrix::unaryOp(
        this, [](float x) { return 1 / x; }, result);
    return result;
}
...
{
    int rows = std::max(op1 != nullptr ? op1->m_Rows : 0, op2 != nullptr ? op2->m_Rows : 0);
    int columns = std::max(op1 != nullptr ? op1->m_Columns : 0, op2 != nullptr ? op2->m_Columns : 0);
    if (rows == 1)
    {
        switch (columns)
        {
...
void IMatrix::binaryOp(IMatrix* op1, IMatrix* op2, std::function<float(float, float)> func, IMatrix* result)
{
...
IMatrix* IMatrix::dot(IMatrix* op1, IMatrix* op2)
{
        ...
        (op2->m_Type == Type::VECTOR || op2->m_Type == Type::MATRIX))
    {
        ...
        reduceToSum(
            op1, op2, [](float x1, float x2) { return x1 * x2; }, result);
        return result;
    }
...
IMatrix* IMatrix::cross(IMatrix* op1, IMatrix* op2)
{
    ...
    if (nrOfVectors == 1)
    {
        ...
        float y2 = op2->get(0, 1);
        float z2 = op2->get(0, 2);
        Vector3D* result = new Vector3D(y1 * z2 - y2 * z1, z1 * x2 - z2 * x1, x1 * y2 - x2 * y1);
