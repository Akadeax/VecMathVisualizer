...
void VecMathListener::exitAssign(VecMath::VecMathParser::AssignContext* ctx)
{
    ...
    if (idToken != nullptr && idToken->getTreeType() != antlr4::tree::ParseTreeType::ERROR && assignToken != nullptr &&
        assignToken->getTreeType() != antlr4::tree::ParseTreeType::ERROR)
    {
...
void VecMathListener::exitValue(VecMath::VecMathParser::ValueContext* ctx)
{
    ...
    else if (ctx->f != nullptr && !m_ExprStack.empty())
    {
        ...
        if (ctxFunc->ID() == nullptr)
        {
            ...
            {
                IMatrix::unaryOp(
                    rawOp, [](float x) { return cos(x); }, result);
            }
            else if (ctxFunc->SIN() != nullptr)
            {
                IMatrix::unaryOp(
                    rawOp, [](float x) { return sin(x); }, result);
            }
            else if (ctxFunc->TAN() != nullptr)
            {
                IMatrix::unaryOp(
                    rawOp, [](float x) { return tan(x); }, result);
            }
            else if (ctxFunc->ACOS() != nullptr)
            {
                IMatrix::unaryOp(
                    rawOp, [](float x) { return acos(x); }, result);
            }
            else if (ctxFunc->ASIN() != nullptr)
            {
                IMatrix::unaryOp(
                    rawOp, [](float x) { return asin(x); }, result);
            }
            else if (ctxFunc->ATAN() != nullptr)
            {
                IMatrix::unaryOp(
                    rawOp, [](float x) { return atan(x); }, result);
            }
            else if (ctxFunc->DEGTORAD() != nullptr)
            {
                IMatrix::unaryOp(
                    rawOp, [](float x) { return x * m_DegToRad; }, result);
            }
            else if (ctxFunc->RADTODEG() != nullptr)
            {
                IMatrix::unaryOp(
                    rawOp, [](float x) { return x * m_RadToDeg; }, result);
            }
            else if (ctxFunc->SQRT() != nullptr)
            {
                IMatrix::unaryOp(
                    rawOp, [](float x) { return sqrt(x); }, result);
            }
        ...
        else
        {
            ...
            if (funcName == "abs")
            {
                result = IMatrix::maxMatrix(rawOp, nullptr);
                IMatrix::unaryOp(
                    rawOp, [](float x) { return abs(x); }, result);
            }
            ...
            else if (funcName == "sind")
            {
                result = IMatrix::maxMatrix(rawOp, nullptr);
                IMatrix::unaryOp(
                    rawOp, [](float x) { return sin(x * m_DegToRad); }, result);
            }
            else if (funcName == "cosd")
            {
                result = IMatrix::maxMatrix(rawOp, nullptr);
                IMatrix::unaryOp(
                    rawOp, [](float x) { return cos(x * m_DegToRad); }, result);
            }
            else if (funcName == "tand")
            {
                result = IMatrix::maxMatrix(rawOp, nullptr);
                IMatrix::unaryOp(
                    rawOp, [](float x) { return tan(x * m_DegToRad); }, result);
            }
            else if (funcName == "asind")
            {
                result = IMatrix::maxMatrix(rawOp, nullptr);
                IMatrix::unaryOp(
                    rawOp, [](float x) { return m_RadToDeg * asin(x); }, result);
            }
            else if (funcName == "acosd")
            {
                result = IMatrix::maxMatrix(rawOp, nullptr);
                IMatrix::unaryOp(
                    rawOp, [](float x) { return m_RadToDeg * acos(x); }, result);
            }
            else if (funcName == "atand")
            {
                result = IMatrix::maxMatrix(rawOp, nullptr);
                IMatrix::unaryOp(
                    rawOp, [](float x) { return m_RadToDeg * atan(x); }, result);
            }
            else
            {
...
void VecMathListener::syntaxError(antlr4::Recognizer* recognizer,
...
void VecMathListener::reportAmbiguity(antlr4::Parser* recognizer,
...
void VecMathListener::reportAttemptingFullContext(antlr4::Parser* recognizer,
                                                  const antlr4::dfa::DFA& dfa,
                                                  size_t startIndex,
                                                  size_t stopIndex,
...
void VecMathListener::reportContextSensitivity(antlr4::Parser* recognizer,
                                               const antlr4::dfa::DFA& dfa,
                                               size_t startIndex,
                                               size_t stopIndex,
                                               size_t prediction,
